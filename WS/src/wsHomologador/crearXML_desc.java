...
public class crearXML_desc {
	...
	public static double suma_linea=0;
    ...
    //    System.out.println("Cabecera-> Importe Total:            "+myCabecera.get_importe_tot());
		...
		for(int i=1; i<_counterDet; i++){
	        ...
	        suma_linea = suma_linea + myDetalle[i].get_valor_unit()*myDetalle[i].get_cantidad();
	    	}
		
		
	    suma_linea = suma_linea-myCabecera.get_tot_desc();
...
//	    H_main.conectar($FILE_NAME);
	...
	public static void writeXML(String $FILE_NAME) throws Exception {
		...
		element.appendChild(UBLExtensions);
			...
			UBLExtensions.appendChild(UBLExtension);
				...
				UBLExtension.appendChild(ExtensionContent);
					...
					AdditionalInformation.appendChild(AdditionalMonetaryTotal_DESC);
						...
						PayableAmount_UBL_DESC.appendChild(document.createTextNode(""+myCabecera.get_tot_desc()));
			...
			//				PayableAmount_UBL_INAFE.appendChild(document.createTextNode(""+myCabecera.get_tot_vta_in()));
	...
	//				PostalAddress2.appendChild(AddressTypeCode);
		...
		element.appendChild(TaxTotal);
			...
			double temp_igv = $SUM_IGV*.1;
			temp_igv = round($SUM_IGV-temp_igv,2);
			
			
			
			TaxAmount.appendChild(document.createTextNode(""+temp_igv));
			...
			TaxTotal.appendChild(TaxSubtotal);
				...
				TaxAmount2.appendChild(document.createTextNode(""+temp_igv));
		...
		element.appendChild(LegalMonetaryTotal);				
			...
			AllowanceTotalAmount.appendChild(document.createTextNode(""+myCabecera.get_tot_desc()));
		...
		//		ChargeTotalAmount.setAttributeNode(attr_currencyID4);					
			...
			// cbc:PayableAmount
			double total_linea = suma_linea*1.18;
			
			Element PayableAmount = document.createElement("cbc:PayableAmount");
			PayableAmount.appendChild(document.createTextNode(""+round(total_linea,2)));
		...
		for (int linea=1; linea<_counterDet; linea++) {	
			...
			element.appendChild(InvoiceLine);		
				...
				InvoiceLine.appendChild(InvoicedQuantity);	
					...
					double monto_linea = round(myDetalle[linea].get_valor_unit()*myDetalle[linea].get_cantidad(),2);
					LineExtensionAmount.appendChild(document.createTextNode(""+monto_linea));
					InvoiceLine.appendChild(LineExtensionAmount);		
			
					// currencyID
					Attr attr_currencyID_ITEM = document.createAttribute("currencyID");
					attr_currencyID_ITEM.setValue("PEN");
					...
					// cac:AlternativeConditionPrice //alex
					...
					PricingReference.appendChild(AlternativeConditionPrice);
						...
						PriceAmount.appendChild(document.createTextNode(""+round(myDetalle[linea].get_valor_unit()*1.18,2)));
						AlternativeConditionPrice.appendChild(PriceAmount);
				
							// currencyID_ALTER
							Attr attr_currencyID_ITEM_ALTER = document.createAttribute("currencyID");
							attr_currencyID_ITEM_ALTER.setValue("PEN");
			...
			//			Amount_AllowanceCharge.setAttributeNode(attr_currencyAllowanceCharge);		
					...
					Invoiced_TaxTotal.appendChild(TaxAmount3);	
						...
						attr_currencyTaxAmount3.setValue("PEN");
					...
					TaxSubtotal2.appendChild(TaxableAmount);	
						...
						attr_TaxableAmount.setValue("PEN");
