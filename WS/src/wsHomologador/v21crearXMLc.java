...
*/
	...
	//	OrderReference.appendChild(OrderReference_ID);
		...
		Signature_ID.appendChild(document.createTextNode($SERIE+"-"+$NUMERO));
		...
		Name_ignature.appendChild(document.createTextNode($RAZON_SOCIAL));
...
//		Name_ignature.appendChild(cdata_Name_ignature);
	...
	//	PartyName_SIG.appendChild(Name_sig);
		...
		URI.appendChild(document.createTextNode($RUC+"-"+$SERIE+"-"+$NUMERO));
...
*/
		...
		*/
				...
				Name_EMISOR.appendChild(document.createTextNode($NOMBRE_COMERCIAL));
				//Node cdata = document.createCDATASection($NOMBRE_COMERCIAL);
		...
		//		RegistrationName_EMISOR.appendChild(cdataRegistrationName_EMISOR);
				...
				AddressTypeCode_EMISOR.appendChild(document.createTextNode("0001"));
...
//		}
		...
		if (_gratuito==0) {
			...
			TaxSubtotal_Header_Gra.appendChild(document.createTextNode(""+Formato._xml(_tax_total)));
...
*/
		...
		if (_gratuito>0) {
			...
			TaxSubtotal_Header_Gratis.appendChild(document.createTextNode(""+Formato._xml(_tax_total)));
...
//		LineExtensionAmount_Header.setAttributeNode(Atr_LineExtensionAmount_Header);
		...
		TaxInclusiveAmount_Header.appendChild(document.createTextNode(""+Formato._xml(_neto)));
	...
	//		ChargeTotalAmount.setAttributeNode(Atr_ChargeTotalAmount);
		...
		for (int linea=1; linea<_counterDet; linea++) {	
			...
			InvoicedQuantity.appendChild(document.createTextNode(""+Formato._xml(myDetalle[linea].get_cantidad())));
			...
			if (myDetalle[linea].get_desc_unit()>0) {
				...
				BaseAmount_Detail.appendChild(document.createTextNode(""+Formato._xml(myDetalle[linea].get_valor_tot())));
				...
				TaxAmount_Detalle.appendChild(document.createTextNode(""+Formato._xml(_igv_detalle)));
			...
			//		TaxSubtotal_detalle_Gra.appendChild(document.createTextNode(""+_igv_detalle));
					...
					Percent_igv.appendChild(document.createTextNode("18.00"));
					...
					TaxScheme_detalle_id_Gra.setAttribute("schemeIDccccc", "UN/ECE 5153");
					TaxScheme_detalle_id_Gra.setAttribute("schemeName", "Tax Scheme Identifier");
					TaxScheme_detalle_id_Gra.setAttribute("schemeAgencyName", "United Nations Economic Commission for Europe");
